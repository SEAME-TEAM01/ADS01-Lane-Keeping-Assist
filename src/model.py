...
def unet_model(HEIGHT, WIDTH, CHANNELS):
  ...
  c5,p5 = encoder(p4, 256)

  # Bottleneck
  b1 = bottleneck(p5, 512)

  # Expansive Path bin
  c6_bin = decoder(b1, c5, 256)
  c7_bin = decoder(c6_bin, c4, 128)
  c8_bin = decoder(c7_bin, c3, 64)
  c9_bin = decoder(c8_bin, c2, 32)
  c10_bin = decoder(c9_bin, c1, 16)

  bin_seg = Conv2D(1, (1,1), activation='sigmoid', name='bin_seg')(c10_bin)

  # Expansive Path inst
  c6_ins = decoder(b1, c5, 256)
  c7_ins = decoder(c6_ins, c4, 128)
  c8_ins = decoder(c7_ins, c3, 64)
  c9_ins = decoder(c8_ins, c2, 32)
  c10_ins = decoder(c9_ins, c1, 16)

  c10_ins = BatchNormalization()(c10_ins)
  c10_ins = ReLU()(c10_ins)
  ins_seg = Conv2D(4, (1,1), activation='sigmoid', name='ins_seg')(c10_ins)
