...
class PiRacerBase:
    ...
    # custom method
    @classmethod
    def _get_channel_active_data(time: float, pwm_controller: PCA9685, channel: int) -> float:
        return (pwm_controller.channels[channel].duty_cycle)
...
class PiRacerStandard(PiRacerBase):
    ...
    # --------------------------------------------------------------------------
    # custom functions
    def standardize(self, x, MIN, MAX):
        median      = (MIN + MAX) / 2
        half_range  = (MAX - median)
        y           = (x - median) / half_range
        return y

    def get_steering_percent(self) -> float:
        return self.standardize(self.get_steering_raw_data(), self.STEERING_MIN, self.STEERING_MAX)
    def get_steering_raw_data(self) -> float:
        return self._get_channel_active_data(self.steering_pwm_controller, self.PWM_STEERING_CHANNEL)
    def get_throttle_raw_data(self):
        return [self.throttle_pwm_controller.channels[self.PWM_THROTTLE_CHANNEL_LEFT_MOTOR_PWM].duty_cycle, self.throttle_pwm_controller.channels[self.PWM_THROTTLE_CHANNEL_RIGHT_MOTOR_PWM].duty_cycle]
    # --------------------------------------------------------------------------
